...
variable "vpc_cidr" {
  ...
  type        = string
  default     = "10.2.0.0/16" # Different CIDR per environment (dev=10.0, staging=10.1, prod=10.2)
}
...
variable "enable_nat_gateway" {
  ...
  type        = bool
  default     = true # Prod needs NAT for private subnet internet access
}

variable "enable_flow_logs" {
  description = "Enable VPC flow logs"
  type        = bool
  default     = true # Always enabled in prod for security auditing
}
...
variable "enable_point_in_time_recovery" {
  ...
  type        = bool
  default     = true # Always on in prod - protects against accidental deletes
}

variable "deletion_protection" {
  description = "Enable deletion protection for DynamoDB table"
  type        = bool
  default     = true # Always on in prod - prevents accidental table deletion
}
...
variable "enable_lifecycle_policy" {
  ...
  type        = bool
  default     = true # Prod should have lifecycle policies to manage costs
}

variable "document_retention_days" {
  description = "Document retention in days"
  type        = number
  default     = 730 # 2 years for production
}

variable "allowed_origins" {
  description = "CORS allowed origins"
  type        = list(string)
  default     = ["https://actions.engsnayl.com"]
}
...
variable "enable_cloudtrail_data_events" {
  ...
  type        = bool
  default     = true # Enabled in prod for full audit trail
}

variable "cloudtrail_retention_days" {
  description = "CloudTrail log retention days"
  type        = number
  default     = 365 # 1 year for prod compliance
}
...
variable "unauthorized_api_calls_threshold" {
  ...
  type        = number
  default     = 10 # Lower threshold in prod (more sensitive)
}
